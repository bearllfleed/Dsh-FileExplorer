...
// ---------------------------------------------------------------------------
// global content search (VS Code-style "search in files")

const MAX_SEARCH_WALK = 10000; // files enumerated per query
const MAX_SEARCH_FILE_BYTES = 1024 * 1024; // per-file content cap
const MAX_SEARCH_RESULT_FILES = 100; // files with matches returned
const MAX_SEARCH_MATCH_LINES = 1000; // total matched lines returned
const MAX_MATCHES_PER_LINE = 10; // highlighted ranges per line
const MAX_LINE_TEXT = 500; // result line text cap
...
async function handleSearch(ctx, req, res) {
  ...
  const real = await resolveAllowed(ctx, requested);
  if (!real) return sendJson(res, 403, { ok: false, error: "denied" });
  if (q === "" || q.length > 500) return sendJson(res, 400, { ok: false, error: "bad-query" });

  const useCase = url.searchParams.get("case") === "1";
  const useWord = url.searchParams.get("word") === "1";
  const useRegex = url.searchParams.get("regex") === "1";
  let source = useRegex ? q : escapeRegExp(q);
  if (useWord) source = "\\b(?:" + source + ")\\b";
  let re;
  try {
    re = new RegExp(source, useCase ? "g" : "gi");
  } catch {
    return sendJson(res, 400, { ok: false, error: "bad-regex" });
  }

  const st = await fs.stat(real).catch(() => null);
  if (!st || !st.isDirectory()) return sendJson(res, 400, { ok: false, error: "not-a-directory" });

  const out = { files: [], truncated: false };
  await walkFiles(real, real, out, MAX_SEARCH_WALK);

  const results = [];
  let matchLines = 0;
  let truncated = false;

  // small concurrency pool: reading thousands of files serially is too slow
  const queue = out.files.slice();
  async function worker() {
    while (queue.length) {
      if (results.length >= MAX_SEARCH_RESULT_FILES || matchLines >= MAX_SEARCH_MATCH_LINES) {
        truncated = true;
        return;
      }
      const f = queue.shift();
      ...
      for (let li = 0; li < lines.length; li++) {
        ...
        const ranges = [];
        let m;
        while ((m = re.exec(line)) !== null && ranges.length < MAX_MATCHES_PER_LINE) {
          ranges.push([m.index, m.index + m[0].length]);
          if (m[0].length === 0) re.lastIndex++; // avoid zero-width loops
        }
        if (ranges.length) {
          matches.push({
            line: li + 1,
            text: line.length > MAX_LINE_TEXT ? line.slice(0, MAX_LINE_TEXT) : line,
            ranges: line.length > MAX_LINE_TEXT ? ranges.filter((r) => r[1] <= MAX_LINE_TEXT) : ranges
          });
          matchLines++;
          if (matches.length >= 200 || matchLines >= MAX_SEARCH_MATCH_LINES) break;
        }
      }
      if (matches.length) {
        if (results.length < MAX_SEARCH_RESULT_FILES) results.push({ path: f.path, rel: f.rel, name: f.name, matches });
        else truncated = true;
      }
    }
  }
  ...
  results.sort((a, b) => a.rel.localeCompare(b.rel, "en"));
  sendJson(res, 200, { ok: true, path: real, results, truncated });
}
...
async function handleRead(ctx, req, res) {
  ...
  // For files above the text cap, don't blanket-truncate binaries (images
  // etc.) that the frontend can still preview via /raw — only mark those
  // truncated that genuinely can't be shown at all.
  if (st.size > MAX_READ_BYTES) {
    const isPreviewableBinary =
      st.size <= MAX_RAW_BYTES && RAW_MIME[path.extname(real).toLowerCase()] != null;
    return sendJson(res, 200, {
      ok: true,
      path: real,
      size: st.size,
      content: null,
      truncated: !isPreviewableBinary,
      binary: isPreviewableBinary
    });
  }
...
/**
 * Batch move: relocate a list of existing items into a target directory.
 * Body: { dir: "<abs target dir>", items: ["<abs from path>", ...] }
 * Paths are kept (only the parent changes); per-item name collisions are
 * reported, not overwritten. Returns one combined result so the client can
 * remap open tabs once instead of after each move.
 */
async function handleMove(ctx, req, res) {
  ...
  if (typeof dir !== "string" || dir === "" || items.length === 0) {
    return sendJson(res, 400, { ok: false, error: "bad-request" });
  }

  const realDir = await resolveAllowed(ctx, dir);
  if (!realDir) return sendJson(res, 403, { ok: false, error: "denied" });
  const isDir = await fs.lstat(realDir).then((s) => s.isDirectory()).catch(() => false);
  if (!isDir) return sendJson(res, 400, { ok: false, error: "not-dir" });

  const results = [];
  const usedNames = new Set();
  for (const from of items) {
    const fail = (error) => results.push({ from, ok: false, error });
    if (typeof from !== "string" || from === "") { fail("bad-request"); continue; }
    const realFrom = await resolveAllowed(ctx, from);
    if (!realFrom) { fail("denied"); continue; }
    const roots = await allowedRoots(ctx);
    if (roots.includes(realFrom)) { fail("denied-root"); continue; }
    if (realFrom === realDir) { fail("same-target"); continue; }
    const stat = await fs.lstat(realFrom).catch(() => null);
    if (!stat) { fail("missing"); continue; }
    if (stat.isDirectory() && isContained(realDir, realFrom)) { fail("descendant"); continue; }

    const name = path.basename(realFrom);
    if (usedNames.has(name)) { fail("duplicate-name"); continue; }
    usedNames.add(name);

    const realTo = path.join(realDir, name);
    const exists = await fs.lstat(realTo).then(() => true).catch(() => false);
    if (exists) { fail("exists"); continue; }
    try {
      await fs.mkdir(path.dirname(realTo), { recursive: true });
      await fs.rename(realFrom, realTo);
      results.push({ from: realFrom, to: realTo, ok: true });
    } catch {
      fail("move-failed");
    }
  }
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    const d11 = ctx.webServer.register({
      kind: "exact",
      path: "/plugin/file-explorer/move",
      handler: (req, res) => handleMove(ctx, req, res)
    });
    const d12 = ctx.webServer.register({
      kind: "exact",
      path: "/plugin/file-explorer/search",
      handler: (req, res) => handleSearch(ctx, req, res)
    });
    return () => {
      d1();
      d2();
