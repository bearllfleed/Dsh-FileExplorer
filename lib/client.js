    ...
    function DirNode(props) {
      ...
      return jsxs(
        "div",
        {
          ...
          children: [
            jsxs("div", {
              className: "fe-node-row fe-dir" + (dragOver ? " fe-drop-target" : "") + (extDragOver ? " fe-ext-drop-target" : ""),
              style: { paddingLeft: 8 + props.depth * 12 },
              draggable: true,
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () {
        function candidateFrom(target) {
          ...
          // 3. Tool-card file link: its text is the (possibly cwd-relative)
          //    path. Require a path-ish shape so random summary buttons are
          //    never hijacked.
          var link = target.closest("button[class*='fileLink' i]");
          if (link) {
            var txt = (link.textContent || "").trim();
            if (txt !== "" && txt.indexOf("\n") === -1 && (txt.charAt(0) === "/" || txt.indexOf("/") !== -1 || txt.indexOf("\\") !== -1 || /^[A-Za-z]:[\\/]/.test(txt))) {
              return txt;
            }
          }
          return null;
