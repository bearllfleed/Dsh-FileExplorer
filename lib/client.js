  ...
  factory: (require) => {
    ...
    function DirNode(props) {
      ...
      if (isOpen && listing && listing.ok) {
        ...
        for (d of listing.dirs || []) {
          children.push(jsx(DirNode, { path: d.path, name: d.name, depth: props.depth + 1, expanded: props.expanded, cache: props.cache, toggle: props.toggle, openFile: props.openFile, openPaths: props.openPaths, treeMenu: props.treeMenu }, d.path));
        }
        for (f of listing.files || []) {
          children.push(jsx(FileRow, { path: f.path, name: f.name, depth: props.depth + 1, openFile: props.openFile, open: props.openPaths.has(f.path), treeMenu: props.treeMenu }, f.path));
        }
      }
      return jsxs(
        "div",
    ...
    function FileTree(props) {
      ...
      return jsx("div", {
        ...
        children: jsx(DirNode, { path: props.root, name: basename(props.root) || props.root, depth: 0, expanded: expanded, cache: cache, toggle: toggle, openFile: props.openFile, openPaths: openPaths, treeMenu: props.treeMenu }, props.root)
      });
    }
          ...
          jsxs("div", {
            ...
            children: [
              ...
              showSettings
                ...
                : (root
                    ? jsx(FileTree, { root: root, editor: props.editor, openFile: props.openFile, treeMenu: props.treeMenu, reloadToken: reloadToken }, root)
    ...
    function apply(ctx) {
      ...
      function remapOpenPaths(fromPath, toPath) {
        ...
        for (var i = 0; i < snap.order.length; i++) {
          ...
          var rel;
          if (p === fromPath) rel = "";
          else if (p.indexOf(fromPath + "/") === 0) rel = p.slice(fromPath.length);
          else continue;
      ...
      function deleteOpenPathsUnder(path) {
        ...
        for (var i = 0; i < snap.order.length; i++) {
          var p = snap.order[i];
          if (p === path || p.indexOf(path + "/") === 0) toClose.push(p);
        }
        ...
        rename: function (path, kind, name) {
          prompt.open({
            ...
            onSubmit: function (newName) {
              ...
              var toPath = joinFsPath(dirnameOf(path), newName);
              postJson(renameUrl(), { from: path, to: toPath }).then(function (r) {
                if (r && r.ok) {
                  tree.reload();
