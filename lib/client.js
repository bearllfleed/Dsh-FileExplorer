window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function FileIcon(props) {
      ...
      var fontSize = label.length >= 4 ? 3.2 : 4.4;
      return jsxs("svg", {
        className: "fe-icon",
        width: props.size || 14,
        height: props.size || 14,
        ...
        children: [
          ...
          label ? jsx("text", { x: 8, y: 11.4, textAnchor: "middle", fontSize: fontSize, fontWeight: 700, fill: color, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", children: label }) : null
        ]
      });
    }
    ...
    function FileRow(props) {
      return jsxs(
        "div",
        {
          ...
          onDragStart: function (e) { props.dragStart(props.path, "file", props.name, e); },
    ...
    function DirNode(props) {
      ...
      if (isOpen && listing && listing.ok) {
        ...
        for (d of listing.dirs || []) {
          children.push(jsx(DirNode, { path: d.path, name: d.name, depth: props.depth + 1, expanded: props.expanded, cache: props.cache, toggle: props.toggle, openFile: props.openFile, openPaths: props.openPaths, treeMenu: props.treeMenu, dragStart: props.dragStart, dragEnd: props.dragEnd, isDragging: props.isDragging, dropOnDir: props.dropOnDir }, d.path));
        }
        for (f of listing.files || []) {
          children.push(jsx(FileRow, { path: f.path, name: f.name, depth: props.depth + 1, openFile: props.openFile, open: props.openPaths.has(f.path), treeMenu: props.treeMenu, dragStart: props.dragStart, dragEnd: props.dragEnd }, f.path));
        }
      }
      return jsxs(
        "div",
        {
          ...
          children: [
            jsxs("div", {
              className: "fe-node-row fe-dir" + (dragOver ? " fe-drop-target" : ""),
              style: { paddingLeft: 8 + props.depth * 12 },
              draggable: true,
              onClick: function () { props.toggle(props.path); },
              onDragStart: function (e) { props.dragStart(props.path, "dir", props.name, e); },
    ...
    function FileTree(props) {
      ...
      return jsx("div", {
        ...
        children: jsx(DirNode, { path: props.root, name: basename(props.root) || props.root, depth: 0, expanded: expanded, cache: cache, toggle: toggle, openFile: props.openFile, openPaths: openPaths, treeMenu: props.treeMenu, dragStart: props.dragStart, dragEnd: props.dragEnd, isDragging: props.isDragging, dropOnDir: props.dropOnDir }, props.root)
      });
    }
          ...
          jsxs("div", {
            ...
            children: [
              ...
              showSettings
                ...
                : (root
                    ? jsx(FileTree, { root: root, editor: props.editor, openFile: props.openFile, treeMenu: props.treeMenu, reloadToken: reloadToken, dragStart: props.dragStart, dragEnd: props.dragEnd, isDragging: props.isDragging, dropOnDir: props.dropOnDir }, root)
    ...
    function apply(ctx) {
      ...
      var dragSource = null;
      function dragStart(path, kind, name, e) {
        dragSource = { path: path, kind: kind, name: name };
        try {
